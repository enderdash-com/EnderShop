...
export function SwitchAccountMenu() {
  ...
  return (
    <DropdownMenuSubContent className="min-w-48 md:min-w-56 max-w-[48svw]">
      ...
      <DropdownMenuItem render={<Link href={`${basePaths.auth}/${viewPaths.auth.signIn}`} />}><CirclePlus className="text-muted-foreground" />{localization.auth.addAccount}</DropdownMenuItem>
    </DropdownMenuSubContent>
  )
}
